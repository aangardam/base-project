...
// import LogoIcon from '../../commons/icon/icon-logo';
// import SignalBarIcon from '../../commons/icon/signal-icon-bar';
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mt-2">
          {loadingRoleAccess ? (
            <div className="flex flex-col justify-center items-center h-96 p-5 gap-4">
              {Array.from({ length: 7 }).map((_, index) => (
                  <Skeleton
                      key={index}
                      className="w-full h-8 mb-2 bg-gray-800"
                  />
              ))}
          </div>
          ):(
            <div className="flex flex-col">
              {dataRoleAccess?.data?.menuAccess.map((menu:any, index:any) => (
                <SidebarMenu 
                  ...
                  menu={menu} 
                  isOpen={sidebarOpen} 
                  // loadingRoleAccess={loadingRoleAccess} 
                />
              ))}
            </div>
          )}
        ...
        <div className="w-full pt-2">
          ...
          <ul className="w-3/4 ml-2 mt-2">
            <li 
              className="flex items-center px-6 py-2 gap-3 hover:bg-blue-600 rounded-md cursor-pointer rounded-md text-white transition-colors"
